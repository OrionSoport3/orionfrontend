...
export const Navbar = ({ruta, lugar, isRestablish, segundoLugar, segundaRuta, color}: Props) => {
  return (
    <div className={`w-screen h-16 md:h-20 flex items-center justify-between absolute smartwatch:fixed  z-40 ${color ?? 'bg-azul-navbar'}`}>
      ...
      : 
      <Link to={lugar} className="text-white font-josefin z-50 w-28 h-9 md:h-10 mr-5 flex justify-center items-center hover:rounded-full hover:bg-gradient-to-r hover:from-blue-300 hover:to-blue-400 transition duration-500 ease-in">{ruta}</Link>
      }
    </div>
  )
}
