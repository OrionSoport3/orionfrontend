...
export const Sidebar: React.FC<SidebarProps> = ({ menuAbierto, toggleMenu }) => {
  return (
    <div className={`transform transition-all duration-500 mt-3 BP1:mt-0 ${menuAbierto ? 'w-[17rem] md:w-[16rem]' : 'w-10 h-10 BP1:h-20 opacity-100'}`}>
      <div className="flex justify-between items-center">
        <h1 className={`flex-grow text-black font-bold text-xl sm:text-2xl md:text-3xl border-b-2 border-black pb-3 font-marcellus transform transition-transform duration-500
          ${menuAbierto ? 'opacity-100' : 'w-[0px] opacity-0'}`}>FILTRAR</h1>
        <button className="list_arrow ml-auto" onClick={toggleMenu}>
      ...
      <div className={`pt-2 md:pt-6 transform transition-all duration-200 ${menuAbierto ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        ...
        <form action="">
          <div className="flex mt-3 justify-start">
            <input type="date" className="bg-white rounded-lg w-full h-9 pl-2 outline-none shadow-lg" />
        ...
        <form action="">
          <div className="flex mt-3 justify-start">
            <input className="bg-white rounded-lg w-full h-9 pl-2 outline-none shadow-lg" type="text" placeholder="Buscar.." />
