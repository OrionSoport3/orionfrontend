...
export const ItemSeleccionable = ({name, isSelected, selected}: Props) => {
    ...
    <button type="button" className={`w-auto h-9 rounded-full  shadow-lg flex flex-row items-center px-4 outline-none my-1 ${isSelected ? 'bg-gris text-white' : 'hover:bg-gray-200 transition-all duration-150 bg-white'} `} onClick={selected}>
        ...
        <h3 className="pl-3 font-manjari">{name}</h3>
    </button>
  )
}
